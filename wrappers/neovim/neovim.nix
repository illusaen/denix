{
  ...
  config.specs.config-path = {
    data = null;
    before = ["INIT_MAIN"];
    config = ''
      local cfgdir = require(vim.g.nix_info_plugin_name)(nil, "settings", "config_directory")
      if type(cfgdir) == "string" then
        vim.opt.packpath:prepend(cfgdir)
        vim.opt.runtimepath:prepend(cfgdir)
        vim.opt.runtimepath:append(cfgdir .. "/after")
        package.path = cfgdir .. "/?.lua;" .. cfgdir .. "/?/init.lua;" .. package.path
      end
    '';
  };
